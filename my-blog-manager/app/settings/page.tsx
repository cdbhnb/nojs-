...
function SettingsContent() {
  ...
  const [playlistImportResult, setPlaylistImportResult] = useState<any>(null);
  ...
  const importPlaylist = async () => {
    const source = formData.playlistSource?.trim();
    if (!source) {
      showToast("请输入网易云歌单 ID 或链接", "warning");
      return;
    }
    ...
    try {
      const configRes = await fetch(`/backend_config.json?t=${Date.now()}`);
      const configData = await configRes.json();
      const res = await fetch(`http://127.0.0.1:${configData.api_port}/api/music/playlist/import`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ playlist: source })
      });
      const data = await res.json();

      if (!data.success) {
        showToast(data.message || "歌单解析失败", "error");
        setPlaylistImportResult({ success: 0, failed: 1, duplicate: 0, message: data.message });
        return;
      }

      const currentIds = new Set((formData.cloudMusicIds || []).map((id: string | number) => String(id)));
      const incomingSongs = data.data?.songs || [];
      const newIds: string[] = [];
      ...
      for (const song of incomingSongs) {
        ...
        if (currentIds.has(id) || newIds.includes(id)) {
          duplicate += 1;
          continue;
        }
        newIds.push(id);
      ...
      handleUpdate('cloudMusicIds', [...formData.cloudMusicIds, ...newIds]);
      setMusicDetails(prev => ({ ...prev, ...nextDetails }));
      setPlaylistImportResult({
        success: newIds.length,
        failed,
        duplicate,
        playlistName: data.data?.name || "网易云歌单"
      });
      showToast(`歌单导入完成：成功 ${newIds.length}，重复 ${duplicate}，失败 ${failed}`, "success");
    } catch (error: any) {
      const message = error?.message || "后端通信失败";
      showToast(message, "error");
      setPlaylistImportResult({ success: 0, failed: 1, duplicate: 0, message });
    } finally {
