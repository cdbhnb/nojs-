import re
from fastapi import APIRouter, Request
import requests

router = APIRouter()


HEADERS = {
    "User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/121.0.0.0 Safari/537.36",
    "Referer": "https://music.163.com/"
}


def extract_playlist_id(value: str):
    value = str(value or "").strip()
    if not value:
        return ""

    if value.isdigit():
        return value

    match = re.search(r"(?:playlist\?id=|[?&]id=)(\d+)", value)
    if match:
        return match.group(1)

    match = re.search(r"playlist/(\d+)", value)
    if match:
        return match.group(1)

    return ""


@router.get("/query/{song_id}")
def query_netease_music(song_id: str):
    """通过网易云公开接口查询歌曲详情"""
    print(f"\n[API] 🎵 收到查询网易云音乐请求, ID: {song_id}")
    try:
        api_url = f"https://music.163.com/api/song/detail/?id={song_id}&ids=[{song_id}]"
        response = requests.get(api_url, headers=HEADERS, timeout=5)

        # 把 HTTP 状态码打出来，如果是 403 就是被网易云拦截了
        print(f"[API] 📡 网易云响应状态码: {response.status_code}")

        data = response.json()

        if data.get("songs") and len(data["songs"]) > 0:
            song = data["songs"][0]
            print(f"[API] ✅ 查询成功: {song['name']} - {song['artists'][0]['name']}")
            return {
                "success": True,
                "data": {
                    "id": song_id,
                    "name": song["name"],
                    "artist": song["artists"][0]["name"],
                    "album": song["album"]["name"],
                    "cover": song["album"]["picUrl"]
                }
            }
        print(f"[API] ❌ 查无此歌 (ID: {song_id})")
        return {"success": False, "message": "未找到该歌曲，可能是 VIP 歌曲或 ID 错误"}

    except Exception as e:
        # 【关键】：在终端里把真正的报错原因打印出来！
        print(f"[API] 💥 网易云接口发生严重错误: {str(e)}")
        return {"success": False, "message": f"后端请求失败: {str(e)}"}


@router.post("/playlist/import")
async def import_netease_playlist(request: Request):
    """解析网易云歌单 ID 或链接，返回可导入的歌曲 ID 列表。"""
    try:
        payload = await request.json()
        source = payload.get("playlist", "")
        playlist_id = extract_playlist_id(source)

        if not playlist_id:
            return {"success": False, "message": "无法解析歌单 ID，请输入网易云歌单 ID 或歌单链接"}

        print(f"\n[API] 🎼 收到网易云歌单导入请求, playlist ID: {playlist_id}")
        api_url = f"https://music.163.com/api/playlist/detail?id={playlist_id}"
        response = requests.get(api_url, headers=HEADERS, timeout=10)
        print(f"[API] 📡 网易云歌单响应状态码: {response.status_code}")
        data = response.json()

        result = data.get("result") or data.get("playlist") or {}
        tracks = result.get("tracks") or []
        track_ids = result.get("trackIds") or []

        songs = []
        if tracks:
            for track in tracks:
                song_id = str(track.get("id", "")).strip()
                if not song_id:
                    continue
                artists = track.get("artists") or track.get("ar") or []
                album = track.get("album") or track.get("al") or {}
                songs.append({
                    "id": song_id,
                    "name": track.get("name") or song_id,
                    "artist": artists[0].get("name", "") if artists else "",
                    "album": album.get("name", ""),
                    "cover": album.get("picUrl", "")
                })
        elif track_ids:
            for item in track_ids:
                song_id = str(item.get("id", "")).strip()
                if song_id:
                    songs.append({"id": song_id, "name": song_id, "artist": "", "album": "", "cover": ""})

        if not songs:
            return {"success": False, "message": "歌单为空，或网易云接口未返回歌曲信息"}

        # 去重但保留歌单顺序
        seen = set()
        unique_songs = []
        for song in songs:
            if song["id"] in seen:
                continue
            seen.add(song["id"])
            unique_songs.append(song)

        return {
            "success": True,
            "data": {
                "playlistId": playlist_id,
                "name": result.get("name", "网易云歌单"),
                "songs": unique_songs
            },
            "message": f"解析成功，共获取 {len(unique_songs)} 首歌曲"
        }
    except Exception as e:
        print(f"[API] 💥 网易云歌单导入失败: {str(e)}")
        return {"success": False, "message": f"歌单导入失败: {str(e)}"}
