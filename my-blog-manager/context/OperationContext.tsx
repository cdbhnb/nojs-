...
interface OperationContextType {
  ...
  addOperation: (op: Partial<Operation> & Pick<Operation, 'type' | 'label'>) => void;
...
export function OperationProvider({ children }: { children: React.ReactNode }) {
  ...
  const addOperation = (op: Partial<Operation> & Pick<Operation, 'type' | 'label'>) => {
    const newOp: Operation = {
      ...op,
      id: op.id || Math.random().toString(36).substr(2, 9),
      description: op.description || op.label,
      payload: op.payload || op.value,
